...
const PogsForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formState.name === "" || formState.ticker_symbol === "" || isNaN(formState.price) || formState.color === "") {
        navigate("/nonexistent-page")
        return;
      }
      await axios.post(
        "http://localhost:8080/pogsform",
        formState
      );
      navigate(`/adminSide`);
    } catch (error: any) {
        ...
        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-2 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              ...
              <button
                className="border rounded-lg p-2 w-full text-white bg-primary-400 hover:bg-primary-900"
                type="submit"
              >
                Submit the form
              </button>
